    ...
    if msg.is_multipart():
        ...
        for part in msg.walk():
            ...
            if not part.is_multipart():
                logging.info("sub-part is not multipart")
                logging.info("GetFileName %s" % part.get_filename())
                #print "@@@@@@@@@@sub-part is not multi-part"
                if part.get_filename():
                    attach_msg(part, mail_num, file_path);
    else:
        #print "  -Not Multipart Mail."
        #print type(msg.get_filename())
        logging.info("Not Multipart mail %s" % msg.get_filename())
        if msg.get_filename():
...
if __name__ == "__main__":
    ...
    if port == "110":
        ...
        # open record file
        fileHandle = open('/mnt/us/DK_System/xKindle/mailsuid','a+')
        mailsUidList = fileHandle.readlines()
        ...
        for item in data:
            try:
                ...
                line = mail_uid + "\n"
                if line not in mailsUidList:
                    logging.info("line not in mailsUidList")
